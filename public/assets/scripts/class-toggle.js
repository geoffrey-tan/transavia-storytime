  ...
  save[i].addEventListener("click", function () {
    ...
    for (var j = 0; j < children.length; j++) {
      if (
        ...
        $(children[j]).attr("src").indexOf("assets/icons/save.svg") != -1
      ) {
        $(".toggle-image").attr("src", "assets/icons/save-a.svg");

        break;
      } else if (
        children[j].nodeName == "IMG" &&
        $(children[j]).attr("src").indexOf("assets/icons/save-a.svg") != -1
      ) {
        $(".toggle-image").attr("src", "assets/icons/save.svg");
      }
    }
  });
}
...
for (var i = 0; i < edit.length; i++) {
  edit[i].addEventListener("click", function () {
    if (
      this.nodeName == "IMG" &&
      $(this).attr("src").indexOf("assets/icons/edit.svg") != -1
    ) {
      $(this).attr("src", "assets/icons/delete.svg");
    } else if (
      this.nodeName == "IMG" &&
      $(this).attr("src").indexOf("assets/icons/delete.svg") != -1
    ) {
      $(this).attr("src", "assets/icons/edit.svg");
    }
  });
}
